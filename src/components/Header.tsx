...
export default function Header() {
  const { sidebarCollapsed, setSidebarCollapsed, searchQuery, setSearchQuery } =
    useLayout();
  ...
  useEffect(() => {
    if (process.env.NODE_ENV !== "production") {
      console.log(
        `[LaunchDarkly] ${FEATURES.REVERSE_IMAGE_SEARCH.name} status:`,
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    if (searchQuery.trim()) {
      ...
      router.push(
        `/?q=${encodeURIComponent(searchQuery)}${isSemantic ? "&is_semantic=true" : ""}`,
      );
    }
  };

  const fetchSuggestions = useCallback(
    async (query: string, semanticState = isSemantic) => {
      if (
        !activeWorkspace ||
        !query.trim() ||
        query.length < 2 ||
      ...
      try {
        const response = await apiFetch<{ assets?: SearchAsset[] }>(
          `/workspaces/${activeWorkspace.id}/search/assets/quick?q=${encodeURIComponent(query)}&limit=6&is_semantic=${semanticState}`,
        );
          ...
          <div className="w-full max-w-2xl relative">
            ...
            >
              ...
              <input
                ...
                placeholder={
                  isSemantic
                    ? "Natural language search aka AI search..."
                    : "Search assets, metadata, tags..."
                }
                value={searchQuery}
                ...
                <div className="flex items-center gap-1">
                  ...
                  <div className="relative">
                    <button
                      ...
                      onBlur={() =>
                        setTimeout(() => setShowSearchInfo(false), 200)
                      }
                  ...
                  <div className="flex items-center ml-1">
                      ...
                      onChange={async (e) => {
                        ...
                        try {
                          ...
                          const response = await apiFetch<{
                            ...
                            `/workspaces/${activeWorkspace.id}/search/visual`,
                            {
                              method: "POST",
                              body: formData,
                            },
                          ...
                          if (response.results) {
                            posthog.capture(
                              "frontend_visual_search_performed",
                              {
                                file_name: file.name,
                                file_size: file.size,
                    ...
                    >
                      <svg
                        ...
                        strokeWidth={1.5}
                        stroke="currentColor"
                        className="w-4 h-4 group-hover/cam:scale-110 transition-transform"
                      >
                        <path
                          strokeLinecap="round"
                          strokeLinejoin="round"
                ...
                <div className="p-2 max-h-[70vh] overflow-y-auto custom-scrollbar">
                  <div className="px-3 py-2 mb-1 flex items-center justify-between">
                    <span className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-[0.2em]">
                      {isSemantic ? "Semantic Matches" : "Suggested Assets"}
                    </span>
                  ...
                  {suggestions.map((asset) => (
                    ...
                    >
                      <div
                        className="h-12 w-12 rounded-xl bg-gray-50 dark:bg-gray-900/40 overflow-hidden flex items-center justify-center shrink-0 border border-gray-200 dark:border-gray-800 group-hover/item:border-blue-500/30 transition-all shadow-sm"
                        title={`Score: ${asset.score?.toFixed(4) || "N/A"}`}
                      >
                          ...
                          <div className="relative h-full w-full">
                            <CustomImage
                              src={
                                ...
                                asset.asset_live_url ||
                                "/logo_transparant.png"
                              }
                              alt=""
                              fill
                              shimmerWidth={48}
