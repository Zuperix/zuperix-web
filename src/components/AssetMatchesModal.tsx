...
export default function AssetMatchesModal({
  asset,
  onClose,
}: {
  asset: Asset;
  ...
  useEffect(() => {
    const fetchMatches = async () => {
      try {
        setLoading(true);
        const data = await apiFetch<Asset[]>(`/assets/${asset.id}/matches`);
        setMatches(data || []);
      } catch (err) {
        console.error('Failed to fetch matches:', err);
        toast.error('Failed to load duplicate matches');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      setMatches(prev => prev.filter(a => a.id !== id));
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 sm:p-6 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="bg-white dark:bg-gray-900 rounded-3xl w-full max-w-4xl shadow-2xl flex flex-col max-h-[85vh] overflow-hidden border dark:border-gray-800">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-5 border-b dark:border-gray-800 flex-shrink-0 bg-white/50 dark:bg-gray-900/50 backdrop-blur-xl">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold dark:text-white">Duplicate Detected</h2>
              <p className="text-xs text-gray-500 mt-0.5 truncate max-w-[300px]">Matches for: {asset.original_name}</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 min-h-0 custom-scrollbar">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20 animate-pulse">
              <div className="h-12 w-12 border-4 border-amber-500/20 border-t-amber-500 rounded-full animate-spin mb-4" />
              <p className="text-sm font-medium text-gray-500">Finding duplicate matches...</p>
            </div>
          ) : matches.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <h3 className="text-lg font-bold dark:text-white">No other matches found</h3>
              <p className="text-sm text-gray-500 mt-1 max-w-xs mx-auto">This asset was marked as a duplicate, but its original might have been deleted.</p>
            </div>
          ) : (
            <div className="space-y-6">
              <div className="flex items-center gap-2 px-1">
                <span className="px-2 py-0.5 rounded bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 text-[10px] font-extrabold uppercase tracking-widest">
                  Found {matches.length} Existing {matches.length === 1 ? 'Asset' : 'Assets'}
                </span>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {matches.map((match) => (
                  ...
                  >
                    ...
                    <div className="mt-3">
                      <p className="text-xs font-bold dark:text-white truncate" title={match.original_name}>
                        {match.original_name}
                      </p>
                      <div className="flex items-center justify-between mt-1">
                        <p className="text-[10px] text-gray-500 font-medium">
                          {formatSize(match.size)} • {new Date(match.created_at).toLocaleDateString()}
                        </p>
                        <button 
        ...
        <div className="px-6 py-4 border-t dark:border-gray-800 flex items-center justify-between gap-3 flex-shrink-0 bg-gray-50/50 dark:bg-gray-900/50">
          <div className="flex items-center gap-2 text-[10px] font-bold text-gray-400 uppercase tracking-widest leading-none">
            ...
            Manage duplicates to save storage space
          </div>
          <button
            onClick={onClose}
